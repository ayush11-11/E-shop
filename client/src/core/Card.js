...
const Card = ({ product,removeButton=false,cartUpdate=false,cartButton=true,showButton=true,de=false,setReload = f => f,reload = undefined }) => {
    ...
    const showCartRemove=()=>{
        return removeButton && (
            <button onClick={()=>{removeItem(product._id); setReload(!reload);} }   className='btn btn-outline-dark-green'> Remove</button>
        )
    }
    return (
            <div className="card">
